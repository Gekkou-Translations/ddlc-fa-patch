init python:
    def RectClusterFunction(trans, st, at, t=0, aw=0, ah=0):
        trans.xpos = int((random.random()-0.5)*aw*2)
        trans.ypos = int((random.random()-0.5)*ah*2)
        return t

    def RectCluster(d, numRects=12, areaWidth=30, areaHeight=30, t=1/60):
        return Fixed(*[Transform(d, function=renpy.partial(RectClusterFunction, t=t, ah=areaHeight, aw=areaWidth)) for i in range(numRects)],
            xysize=(areaWidth, areaHeight))

image n_rects_ghost1:
    RectCluster(Solid("#000"), 4, 15, 5)
    pos (580, 270)
    size (20, 25)
    alpha 0.0
    8.0
